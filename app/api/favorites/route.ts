...
export async function POST(req: NextRequest) {
    ...
    const data = await fs.readFile(FAVORITES_PATH, 'utf-8');
    const favorites: Favorite[] = JSON.parse(data);
    const exists = favorites.some((fav) => fav.user === user && fav.project === project);
    if (!exists) {
...
export async function DELETE(req: NextRequest) {
    const { user, project } = await req.json();
    const data = await fs.readFile(FAVORITES_PATH, 'utf-8');
    let favorites: Favorite[] = JSON.parse(data);
    favorites = favorites.filter((fav) => !(fav.user === user && fav.project === project));
    await fs.writeFile(FAVORITES_PATH, JSON.stringify(favorites, null, 2));
    return NextResponse.json({ success: true });
}
